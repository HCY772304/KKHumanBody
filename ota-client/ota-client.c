...
#include "../ota-client_inc/ota-client-signature-verify.h"
#include "../ota-client_inc/ota-client-page-request.h"
...
#endif
//add by maozj 20200224
#include "../00ikonke-app/general/ikk-ota.h"
//------------------------------------------------------------------------------
// Globals
...
static bool downloadAndVerifyFinish(EmberAfOtaDownloadResult result)
{
  ...
  /*begin add by maozj 20200224 ota update failed deal*/
  if (result == EMBER_AF_OTA_SERVER_ABORTED || result == EMBER_AF_OTA_VERIFY_FAILED \
      || result == EMBER_AF_OTA_ERASE_FAILED){
    kOTAMoudleCallback(OTA_FAILED);//add by maozj 20190123  ��y??����㨹
  }else if (result == EMBER_AF_OTA_DOWNLOAD_AND_VERIFY_SUCCESS){
    kOTAMoudleCallback(OTA_VERITY_SUCCEED);// D��?��3��1|��??????����??�¡��䨬??�̡�y?��??o��???��
  }
  ...
  if (zclStatus == EMBER_ZCL_STATUS_SUCCESS) {
    ...
    sendMessage(ZCL_UPGRADE_END_REQUEST_COMMAND_ID,
                zclStatus,
                EMBER_AF_RUN_UPGRADE_REQUEST_DELAY_MS);
    kOTAMoudleCallback(OTA_FAILED);//add by maozj 20200224  ��y??����㨹
  }
  updateDownloadedZigbeeStackVersionAttribute();
...
static EmberAfStatus imageBlockResponseParse(uint8_t* buffer, uint8_t index, uint8_t length)
{
  ...
  offset = updateCurrentOffset(nextOffset);
      /*begin add by maozj 20200224*/
     if (offset >= 64  && offset <= 128){
       kOTAMoudleCallback(OTA_START); //?a��?��y??,?y����
     }
     /*end add by maozj 20190308*/
#if defined(EMBER_TEST)
  {
...
#else
  /*begin modify by maozj 20200224*/
    uint8_t percentageComplete =  emAfPrintPercentageUpdate("Download", DOWNLOAD_PERCENTAGE_UPDATE_RATE, offset);
    if (percentageComplete >= 98){
      kOTAMoudleCallback(OTA_DOWNLOAD_DONE);
    }
    /*end modify by maozj 20200224*/
  emAfPrintPercentageUpdate("Download",
                            DOWNLOAD_PERCENTAGE_UPDATE_RATE,
                            offset);
#endif
