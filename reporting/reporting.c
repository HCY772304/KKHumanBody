...
#include "../reporting_inc/reporting.h"
    ...
    if (entry.direction == EMBER_ZCL_REPORTING_DIRECTION_REPORTED
      ...
      EmberAfDifferenceType difference
        = emberAfGetDifference(dataRef,
                               ...
                               dataSize);
      /*begin add by maozj 2020*/
    extern bool kZclClusterReportChangeCallback(uint8_t endpoint, EmberAfClusterId clusterId, EmberAfAttributeId attributeId, EmberAfDifferenceType *difference, uint32_t reportableChange);
    if (kZclClusterReportChangeCallback(endpoint, clusterId, attributeId, &difference, entry.data.reported.reportableChange) == true){
      emberAfCorePrintln("Cluster Report EP(%d)", endpoint);
      //difference = 1;
    }else {
      emberAfCorePrintln("Cluster Report EP(%d) is Not Permit", endpoint);
      break;
    }
    /*end add by maozj 2020*/
