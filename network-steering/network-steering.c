...
void emberAfPluginNetworkSteeringFinishSteeringEventHandler(void)
{
  ...
  if (emAfPluginNetworkSteeringStateVerifyTclk()) {
    ...
    emberAfPluginUpdateTcLinkKeyStop();
    /*begin modify by maozj 20181228 ?a??¨¨?¨ª?o¨®?¨¹D?link key¨º¡ì¡ã¨¹leave¨ª?1??¨º¨¬a¡ê? ?a¨¤?¡Á¡é¨º¨ª¦Ì?leavenetwork*/
  #if 0
    emberLeaveNetwork();
  ...
  /*end modify by maozj 20181228*/
    ...
    emberAfDebugPrintln("INTO emberAfPluginNetworkSteeringFinishSteeringEventHandler 222 .");
    emberAfRemoveFromCurrentAppTasks(EMBER_AF_WAITING_FOR_TC_KEY_UPDATE);
  } else {
...
void emberAfPluginUpdateTcLinkKeyStatusCallback(EmberKeyStatus keyStatus)
{
  if (emAfPluginNetworkSteeringStateUpdateTclk()) {
    ...
    switch (keyStatus) {
      ...
      default:
          // Failure!
      /*begin modify by maozj 20190102?a??¨¨?¨ª?o¨®?¨¹D?link key¨º¡ì¡ã¨¹leave¨ª?1??¨º¨¬a¡ê? ?a¨¤?¡Á¡é¨º¨ª¦Ì?leavenetwork*/
      #if 0
      emberLeaveNetwork();
      cleanupAndStop(EMBER_NO_LINK_KEY_RECEIVED);
      #endif
      break;
      /*end modify by maozj 20190102*/
    }
