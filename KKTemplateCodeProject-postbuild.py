...
def S37():
    ...
    if ("iar" == "iar" or "iar" == ""):
        ...
        GCC_TOOLCHAIN_PATH = shortPath(r"file:/D:/IAR7_8/".replace(
            "file:", "").replace("%20", " "))
...
def EBL():
    ...
    print (" ")
    EBL_COMMAND = START_FLAGS + shortPath(r"D:\simplicity studio5\s5\developer\adapter_packs\commander\commander.exe").replace(
...
def GBL():
    ...
    GBL_COMMAND = START_FLAGS + shortPath(r"D:\simplicity studio5\s5\developer\adapter_packs\commander\commander.exe").replace(
...
def OTA():
    if not '"%PROJECT_DIR%\..\\developer\\sdks\\gecko_sdk_suite\\v3.1\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 0x14 --manuf-id 0x1268 --image-type 20506 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL KKTemplateCodeProject"':
        sys.exit(0)
    print (" ")
    ...
    IMAGE_BUILDER = '"%PROJECT_DIR%\..\\developer\\sdks\\gecko_sdk_suite\\v3.1\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 0x14 --manuf-id 0x1268 --image-type 20506 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL KKTemplateCodeProject"'.replace("\t", "\\t")
